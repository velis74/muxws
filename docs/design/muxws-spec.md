...
| M5 | Robustness: fragmentation wired into the send path with the one-unsent-fragment rule and round-robin writer, all three receive-side caps (frame size, `max_payload_bytes` enforced as fragments accumulate, the local concurrency limit answering with `REFUSED`), `PeerRegistry`, `on_frame`, and the reconnect helper on both clients. First production-usable release; the first backchannel can build against. | §4.2-4.3, §7, §9.5, §12, WSM-STM-036/037 |
